...
def grid_search(eta, lamda, depth):
    Train_AUC = np.zeros((len(depth),len(eta)))                             # Define matrices to store accuracy scores as a function
    Test_AUC = np.zeros((len(depth),len(eta)))                              # of learning rate and tree depth to find best hyperparameters 
    Train_accuracy = np.zeros((len(depth),len(eta)))          
    Test_accuracy = np.zeros((len(depth),len(eta)))           
    Exp_sig = np.zeros((len(depth),len(eta)))  
    ...
    for i in range(len(depth)):                                             # Run loops over max depth and learning rates to calculate 
        for j in range(len(eta)):      
            ...
            print('%.3f sigma with %.2f%% accuracy and %.2f AUC on testing, and %.2f%% accuracy and %.2f AUC on training' %(Z, acc_test*100, test, acc_train*100, train))
            Exp_sig[i,j] = Z
            plt.clf() 
...
model = xgbclassifier = xgb.XGBClassifier(
                    ...
                    use_label_encoder=False,
                    n_estimators=120,
                    learning_rate=eta[int(indices[1])],
                    reg_lambda = lamda,
                    predictor = 'cpu_predictor',
                    tree_method = 'hist',
                    scale_pos_weight=sow_bkg/sow_sig,
...
model.save_model(model_dir+'best_full')