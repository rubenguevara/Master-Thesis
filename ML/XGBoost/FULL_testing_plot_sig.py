...
sig_mzp_400 = np.load(np_dir+dm_model+'_mZp_400/sig_pred_'+channel+'.npy')
sig_mzp_600 = np.load(np_dir+dm_model+'_mZp_600/sig_pred_'+channel+'.npy')

bkg_mzp_130 = np.load(np_dir+dm_model+'_mZp_130/bkg_pred_'+channel+'.npy')
bkg_mzp_200 = np.load(np_dir+dm_model+'_mZp_200/bkg_pred_'+channel+'.npy')
bkg_mzp_400 = np.load(np_dir+dm_model+'_mZp_400/bkg_pred_'+channel+'.npy')
bkg_mzp_600 = np.load(np_dir+dm_model+'_mZp_600/bkg_pred_'+channel+'.npy')
...
plt.xlim([0,1])
plt.ylim([min(Y_axis_600)*0.9, max(Y_axis_130)*1.1])
plt.yscale('log')
plt.grid(True)
plt.legend()
plt.ylabel('Expected significance [$\sigma$]')
plt.title("Significance on "+dm_model.split('_')[0]+' '+dm_model.split('_')[1]+" "+channel+", trained network on "+dm_model.split('_')[0]+' '+dm_model.split('_')[1])
