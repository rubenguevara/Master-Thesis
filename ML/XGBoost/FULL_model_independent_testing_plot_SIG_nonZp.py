...
save_txt_path = '../../Plots/Limits/Model_independent/'+met_reg+'/'+dm_model+'/'
...
if dm_model == 'SlepSlep':
    ...
    if channel == 'ee':
        title = "'#tilde{e}#tilde{e} #rightarrow ee #tilde{#chi}_{1}^{0}#tilde{#chi}_{1}^{0} in "+SR+"'"
        m_x = "'m_{ #tilde{e}} [GeV]'"
    if channel == 'uu':
        title = "'#tilde{#mu}#tilde{#mu} #rightarrow #mu#mu #tilde{#chi}_{1}^{0}#tilde{#chi}_{1}^{0} in "+SR+"'"
        m_x = "'m_{ #tilde{#mu}} [GeV]'"
    if channel == 'll':
        m_x = "'m_{ #tilde{l}} [GeV]'"
        title = "'#tilde{l} #tilde{l} #rightarrow ll #tilde{#chi}_{1}^{0}#tilde{#chi}_{1}^{0} in "+SR+"'"

if dm_model == '2HDM':
    m_y = "'m_{a} [GeV]'"
    title = "'tan#beta = "+tanb+" in "+SR+"'"
    m_x= "'m_{H^{-}} [GeV]'"
...
if channel == 'll':
    for dirpath in directories:
        if dm_model == '2HDM':
            if dirpath.split('_')[-2][2:] != tanb: continue
        ...
        if dm_model =='SlepSlep':
            ...
            neut = dirpath.split('_')[-1][1:]
        if dm_model =='2HDM':
            slep = dirpath.split('_')[-3][1:]
            neut = dirpath.split('_')[-4][1:]
