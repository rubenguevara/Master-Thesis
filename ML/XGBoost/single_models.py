...
for dsid_int in dsid_test:
    ...
    dsid_title = dsid_name[0] +' '+ dsid_name[1] +' '+ dsid_name[2] +' '+ dsid_name[3]# +' '+ dsid_name[4]
    # if dsid_name[4] =='$\mu\mu$':
    #     dsid_name[4] = 'uu'
    dsid_save = dsid_name[0] +'_'+ dsid_name[1] + '_mZp_' + dsid_name[3]# +'_'+ dsid_name[4]
    # if dsid in already_done: 
    #     print('Already did', dsid)
    #     continue
    ...
    # particles = dsid_title.split(' ')[-1]
    
    # if particles == '$\mu\mu$':
    #     particles = "uu"
    
    # df_bkg = df_bkgs.copy()
    # df_chnl = df_bkg[df_bkg['Dileptons'] == particles]
    ...
    # xgbclassifier.save_model(model_dir+dsid+'.txt')
    ...
    print('Last data point:', data_hist[-1], '+-', unc_data[-1])
    ...
    print('Last MC bin:', bkg_pred[-1], '+-', unc_bkg[-1])
    # stat_unc_sigs = np.sqrt(sig_pred)
    # syst_unc_sigs = sig_pred*0.3                                            # Assuming 30% systematic uncertainty
    # unc_sig = stat_unc_sigs + syst_unc_sigs
    
    
    ratio = data_hist/bkg_pred
    unc_ratio = ratio*np.sqrt( (unc_data/data_hist)**2 + (unc_bkg/bkg_pred)**2)
    
    print('Last ratio point:', ratio[-1], '+-', unc_ratio[-1])
    # ev_sig = data_hist/(bkg_pred+sig_pred)
    # unc_ev_sig = ev_sig*np.sqrt( (unc_data/data_hist)**2 + ((unc_bkg+unc_sig)/(bkg_pred+sig_pred))**2)
    plt.clf()
    ...
    # ax1.bar(x_axis, 2*stat_unc_bkgs, bottom=bkg_pred-stat_unc_bkgs, fill=False, hatch='XXXXX', width = width, lw=0.0, alpha=0.3, edgecolor='red', label='Stat. unc.')
    ...
    print('Last ratio point:', ratio[-1], '+-', unc_ratio[-1])
    # ax2.plot(x_axis, ev_sig + unc_ev_sig, color='#F42069', linestyle='--')
    # ax2.plot(x_axis, ev_sig - unc_ev_sig, color='#F42069', linestyle='--')
    ax2.grid(axis='y')
    ax2.set_xlim([0,1])
    ax2.set_ylim([0,2.8])
    ...
    plt.savefig(plot_dir+'VAL_test.pdf')
    plt.show()
    ...
    plt.xlabel('XGBoost output')
    plt.savefig(plot_dir+'EXP_SIG_test.pdf')
    plt.show()
