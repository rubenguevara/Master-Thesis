...
print('Doing '+dm_model+" model on",channel,'channel')
...
if dm_model == '2HDM':
    xgbclassifier.load_model(model_dir+'best_Stop.txt')
else: 
...
if dm_model == '2HDM':
    ...
    fig.suptitle('XGBoost output, 2 Higgs Doublet Model validation data with 20 % syst. unc.\n Network trained on full 2HDM + a dataset with $E_{T}^{miss} > 50$ GeV', fontsize='x-large')
