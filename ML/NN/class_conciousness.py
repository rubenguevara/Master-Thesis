...
data_jet1Phi = df_data.pop('jet1Phi')
data_jet2Phi = df_data.pop('jet2Phi')
data_jet1Eta = df_data.pop('jet1Eta')
data_jet2Eta = df_data.pop('jet2Eta')

data_train, data_test = train_test_split(df_data, test_size = 0.1, random_state = 42)

x_mean = pd.read_pickle("../Data/df_values/mean_df.pkl")
x_var = pd.read_pickle("../Data/df_values/var_df.pkl")

# data_test = (data_test - x_mean)/np.sqrt(x_var)
...
dsid_test = [[514560, 514561], [514628, 514629]]#, 514560, 514561, 514564, 514565] 
...
for dsid_int in dsid_test:
    ...
    # network = tf.keras.models.load_model(model_dir+'CLASS_STRUGGLE_WGT_sig_up')
    network = tf.keras.models.load_model(model_dir+'NO_NOTHING')
    # network = tf.keras.models.load_model(model_dir+'CLASS_STRUGGLE_WGT_bkg_down')
    # network = tf.keras.models.load_model(model_dir+'CLASS_STRUGGLE_bkg_down')
    # network = tf.keras.models.load_model(model_dir+'Purely_balancing_bkg_down_evnts')
    ...
    # plot_dir = '../../Plots/NeuralNetwork/FULL/NO_PADDING/'
    # plot_dir = '../../Plots/NeuralNetwork/FULL/NO_WEIGHTS/'
    plot_dir = '../../Plots/TESTING/NeuralNetwork/NO_WEIGHTS/'+dsid_save+'/'
    # plot_dir = '../../Plots/Debug/NeuralNetwork/BALANCED_WEIGHTING/MC_WGT_BKG_DOWN/'+dsid_save+'/'
    # plot_dir = '../../Plots/Debug/NeuralNetwork/BALANCED_WEIGHTING/MC_EVENTS_BKG_DOWN/'+dsid_save+'/'
    # plot_dir = '../../Plots/TESTING/NeuralNetwork/BALANCED_WEIGHTING/ONLY_BALANCE_MC_EVENTS_BKG_DOWN/'+dsid_save+'/'
