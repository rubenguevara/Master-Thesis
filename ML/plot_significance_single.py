...
dsid = str( dsid_DH_HDS_MZ_130[1] )
chnl = 'uu'
model = 'DH'

print('Doing single', model, chnl, 'on', ML)
...
if ML =='NN':
    plot_dir = 'Plots_NeuralNetwork/DSID/'+dsid+'/SIGNIFICANCE/'
    model_dir = 'Models/NN/DSID/WEIGHTED/'+dsid
    # model_type = 'FULL_WEIGHTED'    
    network = tf.keras.models.load_model(model_dir)#+model_type)  
    network_pred_label = network.predict(X_test, batch_size = 1024, use_multiprocessing = True, verbose = 1).ravel()
...
plt.ylabel('Events')
plt.yscale('log')
plt.title("Model output, trained network on "+model+" HDS $m_{Z'}$=130 GeV "+chnl+" model, validation data")
plt.grid(True)
plt.legend()
...
plt.title("Significance, trained network on "+model+" HDS $m_{Z'}$=130 GeV "+chnl+" model")
