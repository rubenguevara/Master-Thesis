...
\\\textit{\textbf{EDIT:}} The reason batch normalization might work best for our case is because when we divide the data by bacthes it might unevenly represent the SM / signal and their ratio. But by using batch normalization it \todo{check if true} takes the average of all the batches creating an closer to real data representation. 
...
and $\lambda=10^{-5}$ \todo{should conduct a new grid search with different $\lambda$ and loss functions}. This grid search had \verb|n_estimators| $\in[10, 100, 500, 1000]$ and depth $\in[3,4,5,6]$. 
