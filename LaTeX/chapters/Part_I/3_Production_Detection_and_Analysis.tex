...
analysis of the \textit{significance} of the \textit{cut signal region} we will also explain how we measure these. To guide us through this process I will use $ZZ^{(*)}$ channel in the discovery of the Higgs Boson in 2012 \cite{Higgs_discovery_2012} as an example of the sucess of these methods.
...
If we take the Higgs discovery, in particular the $H\rightarrow ZZ^{(*)} \rightarrow 4l$ channel, then we can as example straight away make one kinematical cut that will decrease most SM backgrounds, that is requiring that there are four leptons in the final state!
We can also check with the detector (and simulation) that there needs to specifically be muons and/or electrons in the final state. Furthermore the four leptons should respectively have a tranvserse momentum greater than 25, 15, 10, 7 GeV \textit{\textbf{why? these cuts are from ATLAS Open Data, can I find a source for the actual cuts used in the 2012 discovery?}} To summarize the cuts made in the cut and count method, we have:
\begin{itemize}
    \item Single-electron or single muon trigger satisfied
    \item Exacctly four leptons with $p_T >25,15,10,7$ GeV, respectively
    \item Higgs-boson candidates are formed by selecting two same flavour opposite charge lepton pairs
\end{itemize}
Using these cuts, as well as the standard criteria cuts (see Chapter \ref{chap:data_anal}) what remains is to count the remaining events. When counting the events that pass the event selection one usually counts the background events that pass, the data points that pass, and 
also the signal events that pass. For the 2012 Higgs discovery, the Higgs channel with a Higgs mass of 125 GeV was used. The remaning events after these cuts is shown in Figure \ref{fig:Higgs_ZZ}.
\begin{figure}[!ht]
	\centering
...
\begin{figure}[!ht]
	\centering
    \includegraphics[width=0.8\textwidth]{CL_Higgs.jpg}
